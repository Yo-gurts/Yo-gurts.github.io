...
void insertionSort(vector<int> &arr) {
    ...
    for (int i = 1; i < len; i++) {
        currInd = i;
        currVal = arr[i];
        while(currInd > 0 && currVal < arr[currInd-1]) {
...
void shellSort(vector<int> &arr) {
    ...
    for (int gap = len/2; gap > 0; gap = gap/2) {
        ...
        for(int i = gap; i < len; i++) {
            currIdx = i;
            currVal = arr[i];
            while(currInd-gap >= 0 && currVal < arr[currInd-gap]) {
                arr[currInd] = arr[currInd-gap];
                currInd -= gap;
            }
            arr[currIdx] = currVal;
        }
...
void radixSort(vector<int> &arr) {
    ...
    int mod = 10, dev = 1;
    int len = arr.size();
    vector<stack<int>> radix(10, stack<int>());
